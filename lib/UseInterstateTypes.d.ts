export declare type InterstateKey = string | number | symbol;
...
export declare type ReadInterstate<M extends object = never> = ([M] extends [never]
  ? {
...
export declare type SetInterstateParam<T> =
  | ((prevValue: T) => T)
  | (T extends any ? ({} extends T ? never : Exclude<T, (...x: any) => any>) : never);

export declare type SetInterstateSchemaParam<M extends object, K extends keyof M = keyof M> =
  | Pick<M, K>
  | ((prevState: M) => Pick<M, K>);

export declare type ResetInterstate<M extends object = never> = ([M] extends [never]
  ? {
      <PreventExplicitGenericUse extends never, S extends object>(
...
declare type DeclareTypeError = typeof DeclareTypeError;

declare type FilterOut<
  T,
  Filter extends
    | 'must be object'
    | 'must have keys'
...
declare type ProcessFilterBlock<
...
declare type InferGuard<T extends Guard, Guard> = T extends Guard ? T : never;

declare type TypeError<
...
declare type ReadonlyIfArray<T> = T extends any[] ? Readonly<T> : T;

declare type PickWithReadonlyArrays<O, K extends keyof O> = {
  [P in K]: ReadonlyIfArray<O[P]>;
};
